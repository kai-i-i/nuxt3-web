<template>
              ...
              <div>
                      ...
                      <li v-for="cookie in cookies[type]" :key="cookie.id">
                        <div class="cookieControl__ModalInputWrapper">
                          ...
                          <span class="cookieControl__ModalCookieName">
                            {{ getName(cookie.name) }}
                            <span v-if="cookie.description">{{ getDescription(cookie.description) }}</span>
                          </span>
                        </div>
                ...
                <div class="cookieControl__ModalButtons">
                  ...
                  <button 
                    ...
                    v-text="cookies.text.declineAll"
                  />
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    emits: ['cookiesAccepted', 'cookiesDeclined'],
    ...
    async beforeMount() {
      if (this.$cookies.necessary) {
        this.$cookies.necessary.forEach((value, index, array) => {
          if (!value.accepted) array[index].accepted = this.handleAccepted
          if (!value.declined) array[index].declined = this.handleDeclined
        })
      }
  
      if (this.$cookies.optional) {
        this.$cookies.optional.forEach((value, index, array) => {
          if (!value.accepted) array[index].accepted = this.handleAccepted
          if (!value.declined) array[index].declined = this.handleDeclined
        })      
      }
      ...
      if (process.client && this.cookies.colors) {
        let key = null
        const variables = {}
          ...
          this.optionalCookies.forEach((c) => {
            if (c.initialState === true) {
              this.cookies.enabledList.push(
                c.identifier || this.cookies.slugify(this.getCookieFirstName(c.name))
              )
            }
          })
        }
    ...
    methods: {
      ...
      setConsent({type, consent=true, reload=true, declineAll=false}) {
        ...
        const enabledCookies = declineAll 
          ...
          : [
            ...this.optionalCookies.map(
              (c) => c.identifier || this.cookies.slugify(this.getCookieFirstName(c.name))
            ),
          ]
      ...
      getDescription(description) {
        if (typeof description === 'string') 
          return ` ${this.cookies.dashInDescription !== false ? '-' : ''} ${description}`
        else if (description[this.locale]) 
          return ` ${this.cookies.dashInDescription !== false ? '-' : ''} ${description[this.locale]}`
        return ''
      },
      ...
      async setTexts(isChanged = false) {
        if (process.client) {
          ...
          try {
            /* @vite-ignore */
            module = await import(`../locale/${this.locale}.js`)
          } catch (e) {
